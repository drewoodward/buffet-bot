...
description: Chart and options analysis on TradingView and Schwab. Produces Buy/Hold/Sell with entry, stop, targets and an invalidation condition. Use for any per-ticker or broad-market technical read.
tools: Bash, Read, Glob, Grep, mcp__tradingview__*, mcp__schwab__*
model: inherit
---

<!-- Tool access above is deliberate. No WebSearch, no WebFetch, no Webull. This agent cannot read news, and that is the design: when it disagrees with the News Agent, the disagreement is real information rather than one agent having been told the other's answer. -->
